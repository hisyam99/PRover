...
from transformers import (
    WEIGHTS_NAME, RobertaConfig, RobertaTokenizer, AdamW, WarmupLinearSchedule
)

from model import RobertaForRRWithNodeEdgeLoss
from utils import (compute_metrics, output_modes, processors, convert_examples_to_features_RR)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss, qa_loss, node_loss, edge_loss = outputs[:4]
...
def evaluate(args, model, tokenizer, processor, prefix="", eval_split=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        args.eval_batch_size = args.per outings_dirs):
        eval_dataset, examples = load_and_cache_examples(args, eval_task, tokenizer, evaluate=True, eval_split=eval_split)
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating", mininterval=10, ncols=100):
            ...
            if preds is None:
                ...
                if not eval_split == "test":
                    ...
                    out_edge_label_ids = np.append(out_edge_label_ids, inputs['edge_label'].detach().cpu().numpy(), axis=0)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, eval_split="train"):
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features_RR(
            examples, label_list, args.max_seq_length, args.max_node_length, args.max_edge_length,
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_proof_offset, all_node_label, all_edge_label, all_label_ids)
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default=None, type=str, required=True,
                        help="Path to pre-trained model or shortcut name (e.g., roberta-large)")
